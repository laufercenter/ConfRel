...
methods. In this work, an improved \Alberto{Arijit: what is the improvement??} version of the confinement approach is used to calculate absolute
=======
conformations is particularly difficult and computationally expensive task with existing
methods. In this work, confinement approach is used to calculate absolute
>>>>>>> 63f47f1c44813a2bcb8b0bf01a1c508ce54de50e
...
transition path and it is fast to compute. The free energy calculated can be decomposed into per residue contribution in a 
approximate way. Per residue free energy allow us
to identify the reason behind conformational preference of the biomolecules. Through out the article we show its use in different
...
folds). This sequence dependent conformational preference and per residue free energy decomposition set the stage for the use of this 
...
trapped in a local minima for a considerable time and trapped in a local energy minima
and one can mistakenly identify the misfolded state as the native state.
...
Decativated Morphing \cite{Park2008}, Orthogonal Space Random Work \cite{Zheng2008}, Confinement 
Method \cite{Tyka2006}, \cite{Cecchini2009} have came out for calculation of free energy method. 
In this work we have applied the confinement method which was originally devoled by Tyka et. al. \cite{Tyka2006} and
Cecchini et. al. \cite{Cecchini2009}. This method relies on the fact that free energy is a state function and 
thus independent of path. It uses a thermodynamic cycle where first, non-harmonic degrees of
...
of CASP9 competition and tested whether it can predict the most stable conformation of the protein based 
on its free energy. The remarkable ability of confinement method to identify residues which 
stabilizes/destabilizes a particular conformation from per residue free energy analysis set the stage
for future application of this method in protein design.
...
energy of the experimentally determined structure. As
...
We decomposed the calculated free energy into per residue decomposition as described in method section. This help us for
identification of important residues that stabilizes a particular conformation. 
...
the fold, where as residues 1-8 and 52-56 are unstructured in $3 \alpha$, but form $\beta$ strand in $4 \beta + \alpha$ structure.
Most of the amino acid residues in the region 1-9 favors $4 \beta + \alpha$ structure and can also seen from the plot 
Figure~\ref{fig:orban_full}(F). Here, we try to explain some of the largest peak in that plot. The residues Leu-5 and 
Leu-7 favors $4 \beta + \alpha$ structure as these hydrophobic residues are oriented towards the protein hydrophobic 
...
it more stability compare to the $4 \beta + \alpha$ structure (Figure~\ref{fig:orban_full}(B)). The peak 
at position 14 is due to the hydrophilic Glu, which is completely
exposed to the solvent in $4 \beta + \alpha$ conformation as shown in (Figure~\ref{fig:orban_full}(E)), but only partially exposed 
