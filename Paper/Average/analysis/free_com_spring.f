...
c
        ntot=56
c
        open(unit=13,file='confinement_Alpha_GB95',status='old')
c
        do i=1,ntot
           read(13,*)nk(i),y1(i)
        end do
c
        open(unit=14,file='confinement_Beta_GB95',status='old')
c
        do i=1,ntot
           read(14,*)nk(i),y2(i)
        end do
c 
         ...
         open(unit=18,file='residue_Alpha_GB95',status='old')
c
        do i=1,ntot
...
c
         open(unit=19,file='residue_Beta_GB95',status='old')
c
        do i=1,ntot
           read(19,10)nk(i),z6(i),z7(i),z8(i),z9(i),z10(i)
        end do
c
        open(unit=21,file='enthalpy_spring_Alpha_GB95',status='old')
c
        do i=1,ntot
           read(21,*)nk(i),az(i)
        end do
c
        open(unit=22,file='enthalpy_spring_Beta_GB95',status='old')
c
        do i=1,ntot
           read(22,*)nk(i),bz(i)
        end do
c
         open(unit=16,file='output_GB95',status='unknown')
c
         do i=1,ntot
